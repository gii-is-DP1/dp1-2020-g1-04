...
<petclinic:layout pageName="animales">
	...
	<table class="table table-striped">
		...
		<tr>
			<th>Centro</th>
			<td><b><c:out value="${animal.centroDeAdopcion.nombre}" /></b></td>
		</tr>
		<tr>
	...
	<sec:authorize access="hasAuthority('duenoadoptivo')">
		<c:if test="${!animal.adoptado}">
			...
			<a href="${fn:escapeXml(editUrl)}" class="btn btn-default">Solicitar
				su adopción</a>
		</c:if>
	</sec:authorize>
	...
	<sec:authorize access="hasAnyAuthority('director')">
		...
		<c:if test="${animal.adoptado}">
			...
			<a href="${fn:escapeXml(reiUrl)}" class="btn btn-default">Reincorporar
				Animal</a>
		</c:if>
	</sec:authorize>
				...
				<table id="enfermedadesTable" class="table table-striped">
					...
					<tbody>
						<c:forEach items="${animal.enfermedades}" var="enfermedad">
							<tr>
								<td><c:out value="${enfermedad.nombre}" /></td>
								<td><c:out value="${enfermedad.comienzo}" /></td>
								<td><c:if test="${enfermedad.curado}">
										<c:out value="Si" />
									</c:if> <c:if test="${!enfermedad.curado}">
										<c:out value="No" />
									</c:if></td>
								<td><a href="/enfermedad/show/${enfermedad.id }">Ver</a></td>

							</tr>
						</c:forEach>
					</tbody>
				</table> <spring:url value="/enfermedad/nuevo/{animalId}" var="editUrl">
					<spring:param name="animalId" value="${animal.id}" />
				</spring:url> <a href="${fn:escapeXml(editUrl)}" class="btn btn-default">Añadir
					Enfermedad</a>
			</th>
		</tr>
	</sec:authorize>
